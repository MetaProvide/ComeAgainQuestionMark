        ...
        for i in range(0, total_duration):
            with sr.AudioFile(input_file_name) as source:
                try:
                    ...
                    f.write(generate_timestamp(seconds_passed) +
                            ': ' + transcribed_chunk)
                    f.write(separator)
                except sr.UnknownValueError:
                    print(
                            "[{0}] Google Recognition Engine could not understand audio chunk".format(i))
                    f.write('------ ')
                except sr.RequestError as e:
                    print("[{0}] Could not request results from Google Speech Recognition service".format(
                        i), e)
            bar()
        f.close()
